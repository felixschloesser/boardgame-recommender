...
def test_missing_liked_games_raise_with_suggestions(
...
def test_candidate_pool_empty_returns_no_results(
    sample_embedding, recommendation_config
):
...
def test_recommendations_are_deterministic_given_seed(
    sample_embedding, recommendation_config
):
...
def test_result_formatting_converts_numeric_fields(
    sample_embedding, recommendation_config
):
    results = recommend_games(
        embedding=sample_embedding,
...
def test_validate_query_rejects_invalid_inputs(recommendation_context):
    ...
    with pytest.raises(ValueError, match="player_count must be positive"):
        context.validate_query(
            ["Alpha"], player_count=0, available_time_minutes=10, amount=1
        )
    with pytest.raises(ValueError, match="available_time_minutes must be positive"):
        context.validate_query(
            ["Alpha"], player_count=1, available_time_minutes=0, amount=1
        )
    with pytest.raises(ValueError, match="amount must be positive"):
        context.validate_query(
            ["Alpha"], player_count=1, available_time_minutes=10, amount=0
        )
...
def test_build_preference_vectors_with_dynamic_centroids(
    ...
    monkeypatch.setattr("boardgames_cli.recommend._run_kmeans", fake_run_kmeans)
